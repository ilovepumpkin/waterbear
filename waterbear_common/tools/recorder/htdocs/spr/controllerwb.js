...
Language.prototype.translateFromSahi = function(s) {
	return s;
};
Language.prototype.translateToSahi = function(s) {
	return s;
};
...
Controller.prototype.getServerVar = function(name, isGlobal, isDelete) {
	try {
		var v = this.sendToServer("/_s_/dyn/SessionState_getVar?name=" + encodeURIComponent(name) + "&isglobal=" + ( isGlobal ? 1 : 0) + "&isdelete=" + ( isDelete ? 1 : 0));
		return eval("(" + v + ")");
	} catch (e) {
	}
};
		...
		try {
			if (msg.mode == "PLAYBACK_LOG_REFRESH") {
				...
				if (msg.accessor && msg.mode != "RE_ID") {
					// msg.accessor =
					// this.language.translateFromSahi(msg.accessor);
				...
				// this.populateOptions($("alternatives"),
				// this.language.translateArrayFromSahi(msg.alternatives));
...
Controller.prototype.getSelectedAssertion = function() {
	...
	var declaredVar = waterbear().findVarByLineCodeStr(accessor);
	var declStat = "";
	var accessorVarName;
	if (declaredVar) {
		accessorVarName = declaredVar.name;
	} else {
		var cl = waterbear().createCLFromStr(accessor);
		declStat = cl.genFullCode();
		var newVar = waterbear().regNewVar(cl);
		accessorVarName = cl.varName;
	}

	var rbValue;
	if (s.indexOf("assertEquals") != -1) {
		rbValue = rb().rbStat(value);
		if (rbValue && !sahi().contains(rbValue, "getString")) {
			rbValue = '"' + rbValue + '"';
		}
	}

	s = s.replace(/<accessor>/g, accessorVarName).replace(/<value>/g, rbValue ? rbValue : this.quote(value));
	s = (declStat == "" ? "" : declStat + ";") + s;
...
Controller.prototype.setServerVar = function(name, value, isGlobal) {
	var url = "/_s_/dyn/SessionState_setVar?" + "sahisid=" + this.sahisid + "&name=" + encodeURIComponent(name) + "&value=" + encodeURIComponent(this.toJSON(value)) + "&isglobal=" + ( isGlobal ? 1 : 0);
	// alert(url);
...
Controller.prototype.isIE = function() {
	return navigator.appName == "Microsoft Internet Explorer";
};
Controller.prototype.isFF3 = function() {
	return navigator.userAgent.match(/Firefox\/3/) != null;
};
Controller.prototype.isFF = function() {
	return navigator.userAgent.match(/Firefox/) != null;
};
Controller.prototype.isChrome = function() {
	return navigator.userAgent.match(/Chrome/) != null;
};
Controller.prototype.isSafariLike = function() {
...
Controller.prototype.onRecordMouseOver = function() {
	if (!this.recording)
		$('record').style.border = "1px outset white";
};
...
function waterbear() {
	return window.opener.top._waterbear;
}

function rb() {
	return window.opener.top.$Messages;
}
...
Controller.prototype.getSelectedText = function(wa) {
	...
	if (sel == "")
		sel = wa.value ? wa.value : wa.innerHTML;
...
Controller.prototype.resize = function(el, x, y) {
	if (x != 0)
		el.style.width = parseInt(el.style.width) + x;
	if (y != 0)
		el.style.height = parseInt(el.style.height) + y;
};
